...
makeCacheMatrix <- function(matrix = matrix()) {
  ...
  inverse<-NULL #we start off by setting the inverse as Null so that we can set the inverse later
  #now we create a set function that sets the matrix value
  set<-function(y){
    matrix<<-y #when set function is called it sets the input matrix or updates it
    inverse<<-NULL #obviously when the matrix value is updated the inverse has to be calculated
...
cacheSolve <- function(special_matrix, ...) {
        ...
        inverse<-special_matrix$get_inverse() #this gets the inverse from the cache
        if(!is.null(inverse)){
        ...
        data<-special_matrix$get()#we get the matrix
